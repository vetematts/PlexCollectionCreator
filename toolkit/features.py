...
def run_manual_mode(pause_fn):
    ...
    collection_name = read_line(Fore.LIGHTBLACK_EX + "Enter a name for your new collection (Esc to cancel): " + Fore.RESET)
    if collection_name is None: return None, None
...
def run_franchise_mode(tmdb, pause_fn):
    ...
    if not tmdb:
        ...
        choice = pick_from_list_case_insensitive("\n" + Fore.LIGHTBLACK_EX + "Select a franchise (Esc to cancel): ", franchises_data.keys())
        ...
        choice = pick_from_list_case_insensitive("\n" + Fore.LIGHTBLACK_EX + "Select a franchise (Esc to cancel): ", constants.KNOWN_FRANCHISES.keys())
        if choice is None: return None, None
        try:
...
def run_studio_mode(tmdb, config, pause_fn):
    ...
    if mode == "ESC": return None, None, False
    ...
    if mode == "1":
        ...
        try:
            ...
            items = [item for item in all_items if getattr(item, 'studio', None) and studio_query.lower() in item.studio.lower()]
    ...
    if mode == "3":
        ...
        if choice is None: return None, None, False
        ...
        if not collection_name or not collection_name.strip(): collection_name = choice
    ...
    if not tmdb:
        ...
        choice = pick_from_list_case_insensitive("\n" + Fore.LIGHTBLACK_EX + "Select a studio (Esc to cancel): ", studios_data.keys())
        if choice is None: return None, None, False
        titles = studios_data.get(choice, [])
    else:
        pretty_names = [k.upper() if k in ("mcu", "dceu") else k.title() for k in constants.STUDIO_MAP.keys()]
        print_grid(pretty_names, columns=3, padding=24, title=Fore.GREEN + "\nAvailable Studios:")
        choice_pretty = pick_from_list_case_insensitive("\n" + Fore.LIGHTBLACK_EX + "Select a studio (Esc to cancel): ", pretty_names)
        if choice_pretty is None: return None, None, False

        studio_info = constants.STUDIO_MAP[choice_pretty.lower()]
        try:
            titles = tmdb.discover_movies(company_id=studio_info.get("company"), keyword_id=studio_info.get("keyword"))
        except Exception as e:
...
def run_poster_tool(config, pause_fn):
    ...
    if choice == "3" or choice == "ESC": return
    ...
    try:
        ...
        if choice == "1":
            ...
            col_name = pick_from_list_case_insensitive("\n" + Fore.LIGHTBLACK_EX + "Select a collection (Esc to cancel): ", col_names)
